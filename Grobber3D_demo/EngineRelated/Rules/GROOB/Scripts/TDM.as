...
shared class TDMSpawns : RespawnSystem
{
	...
	{
		if(s32(p_info.team) == 0)
		{
			Vec2f[] respawnPositions;
			if (!getMap().getMarkers("green main spawn", respawnPositions))
			{
				warn("Green spawns not found!");
				return Vec2f(0, 0);
			}
			else
			{
				int spawnindex = XORRandom(997) % respawnPositions.length;
				return respawnPositions[spawnindex];
			}
		}
		
		else if(s32(p_info.team) == 1)
		{
			Vec2f[] respawnPositions;
			if (!getMap().getMarkers("red main spawn", respawnPositions))
			{
				warn("Red spawns not found!");
				return Vec2f(0, 0);
			}
			else
			{
...
shared class TDMCore : RulesCore
{
	...
	{
		TDMPlayerInfo p(player.getUsername(), player.getTeamNum(), "player");
		players.push_back(p);
	...
	void SetupBases()
	{
		...
		{
			Vec2f[] temp;
			if (!getMap().getMarkers("green main spawn", temp))
			{
				warn("TDM: Green spawn marker not found on map");
			}
			else
			{
				printf("TDM: Green spawn set up");
			}
			
			if (!getMap().getMarkers("red main spawn", temp))
			{
				warn("TDM: Red spawn marker not found on map");
			}
			else
			{
				printf("TDM: Red spawn set up");
			}
		}
