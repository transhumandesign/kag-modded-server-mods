// BombSatchel.as
// by Skinney
// modified for The Waste Kind by 8x
// based on sticky nade
...
void onInit(CBlob@ this)
{
	...
	// 8x - uncommented for players not to be slowed down when holding it lit
	// this also means power throws
	//this.Tag("medium weight");

	// used by Explosion.as
	this.set_f32("explosive_radius", 45.0f); 		// smaller than sticky nade
	this.set_f32("explosive_damage", 3.5f); 		// same
	this.set_bool("explosive_teamkill", true); 		// same

	// 8x - testing how not to damage anything but players
	...
	// used by this script
	// 8x - let's say frag grenades don't emit light and should have no particles when activated
...
{
	// 8x - don't want it to burn players
...
void onCommand(CBlob@ this, u8 cmd, CBitStream @params)
{
	...
	// no stickingfor this fishy grenade
	//this.Tag("spiky");

	// turn light on
	// no light for this fishy grenades
	this.SetLight(false);
	...
	// No sparkle sound for frag nades
	sprite.SetAnimation("lit");
	//sprite.SetEmitSoundPaused(true);
	
	// cling sound from M1 Garand clip that should fit
	sprite.PlaySound("/Ding.ogg");
}
