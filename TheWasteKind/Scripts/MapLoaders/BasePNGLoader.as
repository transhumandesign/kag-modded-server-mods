// Look for "Cross fingers from here" to see what's been added :^)
// BasePNGLoader.as
...
class PNGLoader
{
	...
	void handlePixel(SColor pixel, int offset)
	{
		...
		//concrete
		//else if(pixel == color_tile_concrete)
		//{
		//	map.SetTile(offset, CMap::tile_concrete);
		//	offsets[concrete_offset].push_back(offset);
		//}
		...
		else if (pixel == color_plat_wood_back_up) //cross fingers
		{
			spawnBlob( map, "plat_wood_back", offset, 255, true );
			offsets[autotile_offset].push_back( offset );
		}
		...
		else if (pixel == color_torch) //cross fingers from here
		{
			spawnBlob( map, "torch", offset, 255, true );
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_torch_r) //cross fingers from here
		{
			spawnBlob( map, "torch_r", offset, 255, true );
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_bumper_piston_up) //cross fingers from here
		{
			spawnBlob( map, "bumper_piston", offset, 255, true );
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_bumper_piston_right)
		{
			CBlob@ blob = spawnBlob(map, "bumper_piston", offset, 255, false);
			offsets[autotile_offset].push_back(offset);
			blob.setAngleDegrees(90.0f);
			blob.getShape().SetStatic(true);
		}
		else if (pixel == color_bumper_piston_down)
		{
			CBlob@ blob = spawnBlob( map, "bumper_piston", offset, 255, false );
			offsets[autotile_offset].push_back( offset );
			CShape@ shape = blob.getShape();
		...
		else if (pixel == color_bumper_piston_left)
		{
			CBlob@ blob = spawnBlob( map, "bumper_piston", offset, 255, false );
			offsets[autotile_offset].push_back( offset );
			CShape@ shape = blob.getShape();
		...
		else if (pixel == color_solid_1_up) // cross fingers from here
		{
			spawnBlob( map, "solid_1", offset, 255, true );
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_stone_brick_up) // cross fingers from here
		{
			spawnBlob( map, "stone_brick", offset, 255, true );
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_balcony_up) // cross fingers from here - make it rotated as well for thin cover walls
		{
			spawnBlob( map, "balcony", offset, 255, true );
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_balcony_right)
		{
			CBlob@ blob = spawnBlob(map, "balcony", offset, 255, false);
			offsets[autotile_offset].push_back(offset);
			blob.setAngleDegrees(90.0f);
			blob.getShape().SetStatic(true);
		}
		else if (pixel == color_balcony_down)
		{
			CBlob@ blob = spawnBlob( map, "balcony", offset, 255, false );
			offsets[autotile_offset].push_back( offset );
			CShape@ shape = blob.getShape();
		...
		else if (pixel == color_balcony_left)
		{
			CBlob@ blob = spawnBlob( map, "balcony", offset, 255, false );
			offsets[autotile_offset].push_back( offset );
			CShape@ shape = blob.getShape();
		...
		else if (pixel == color_healer)
		{
			spawnBlob( map, "healer", offset, 255, true );
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_triangle_up) // cross fingers from here
		{
			spawnBlob( map, "triangle", offset, 255, true );
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_triangle_right)
		{
			CBlob@ blob = spawnBlob(map, "triangle", offset, 255, false);
			offsets[autotile_offset].push_back(offset);
			blob.setAngleDegrees(90.0f);
			blob.getShape().SetStatic(true);
		}
		else if (pixel == color_triangle_down)
		{
			CBlob@ blob = spawnBlob( map, "triangle", offset, 255, false );
			offsets[autotile_offset].push_back( offset );
			CShape@ shape = blob.getShape();
		...
		else if (pixel == color_triangle_left)
		{
			CBlob@ blob = spawnBlob( map, "triangle", offset, 255, false );
			offsets[autotile_offset].push_back( offset );
			CShape@ shape = blob.getShape();
		...
		else if (pixel == color_longtriangler_up) // cross fingers from here, Long Triangle 0,8 - 16,0
		{
			spawnBlob( map, "longtriangler", offset, 255, true );
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_longtriangler_right)
		{
			CBlob@ blob = spawnBlob(map, "longtriangler", offset, 255, false);
			offsets[autotile_offset].push_back(offset);
			blob.setAngleDegrees(90.0f);
			blob.getShape().SetStatic(true);
		}
		else if (pixel == color_longtriangler_down)
		{
			CBlob@ blob = spawnBlob( map, "longtriangler", offset, 255, false );
			offsets[autotile_offset].push_back( offset );
			CShape@ shape = blob.getShape();
		...
		else if (pixel == color_longtriangler_left)
		{
			CBlob@ blob = spawnBlob( map, "longtriangler", offset, 255, false );
			offsets[autotile_offset].push_back( offset );
			CShape@ shape = blob.getShape();
		...
		else if (pixel == color_longtrianglel_up) // cross fingers from here, Long Triangle 0,0 - 16,8
		{
			spawnBlob( map, "longtrianglel", offset, 255, true );
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_longtrianglel_right)
		{
			CBlob@ blob = spawnBlob(map, "longtrianglel", offset, 255, false);
			offsets[autotile_offset].push_back(offset);
			blob.setAngleDegrees(90.0f);
			blob.getShape().SetStatic(true);
		}
		else if (pixel == color_longtrianglel_down)
		{
			CBlob@ blob = spawnBlob( map, "longtrianglel", offset, 255, false );
			offsets[autotile_offset].push_back( offset );
			CShape@ shape = blob.getShape();
		...
		else if (pixel == color_longtrianglel_left)
		{
			CBlob@ blob = spawnBlob( map, "longtrianglel", offset, 255, false );
			offsets[autotile_offset].push_back( offset );
			CShape@ shape = blob.getShape();
		...
		else if (pixel == color_woodenb_door_h_noteam) // cross fingers
		{
			spawnBlob( map, "woodenb_door", offset, 255, true );
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_wheel_door_v_noteam) // cross fingers
		{
			spawnBlob( map, "wheel_door", offset, 255, true );
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_mid_door_h_noteam) // cross fingers
		{
			spawnBlob( map, "mid_door", offset, 255, true );
			offsets[autotile_offset].push_back( offset );
		}

		// item spawners from Diprog, to spawn explosives and heal at strategic spots
		else if (pixel == color_red_itemspawner)
		{
			spawnBlob( map, "itemspawner", offset, 1, true);
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_blue_itemspawner)
		{
			spawnBlob( map, "itemspawner", offset, 0, true);
			offsets[autotile_offset].push_back( offset );
		}
	...
	void handleOffset(int type, int offset, int position, int count)
	{
		...
		/*else if(type == concrete_offset)
		{
			// store tile map width, it will be used frequently in adjacent checks
			u32 map_width = map.tilemapwidth;

			// init variables
			u8 flags = 0;
			u16 base = tile_concrete;

			// for every index in array, check adjacent tiles then set tile
			u16[]@ concrete = offsets[concrete_offset];
			for(u32 i = 0; i < concrete.length; i++)
			{
					// clear flags so it's fresh for every iteration
					flags = 0;

					//populate flags based on adjacent
					// this doesn't check edge of map, will need to perform additional checks for that
					flags |= isTileConcrete(concrete[i] - map_width)? north : 0;
					flags |= isTileConcrete(concrete[i] + map_width)? south : 0;
					flags |= isTileconcrete(concrete[i] - 1)? west : 0;
					flags |= isTileConcrete(concrete[i] + 1)? east : 0;

					//set tile based on flags
					map.server_SetTile(map.getTileWorldPosition(concrete[i]), base + flags);
...
void PlaceMostLikelyTile(CMap@ map, int offset)
{
	...
	if(!upEmpty)
	{
		...
		//attempt to make functionality for the custom tiles and make triangles have a blank background		
		else if(up == CMap::tile_stone_triangle || up == CMap::tile_blank_light_stopper || down == CMap::tile_stone_triangle || down == CMap::tile_blank_light_stopper ||
			left == CMap::tile_stone_triangle || left == CMap::tile_blank_light_stopper || right == CMap::tile_stone_triangle || right == CMap::tile_blank_light_stopper)
		{
