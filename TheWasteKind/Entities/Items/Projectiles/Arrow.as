...
const f32 arrowFastSpeed = 15.0f;
...
void onInit(CBlob@ this)
{
	...
	// in ArrowHitMap
	this.server_SetTimeToDie(3); // Set to 3 seconds, bullets dissappearing will avoid the rain spam shot
	...
	//set a random frame
	{
		Animation@ anim = sprite.addAnimation("arrow", 3, true);
		anim.AddFrame(0);
		anim.AddFrame(1);
		anim.AddFrame(2);
		anim.AddFrame(3);
...
void onTick(CBlob@ this)
{
	...
	if (!this.hasTag("collided")) //we haven't hit anything yet!
	{
		...
		if (shape.vellen > 0.0001f)
		{
			if (shape.vellen > 13.5f)
				...
				shape.SetGravityScale(0.3f);//(Maths::Min(1.0f, 1.0f / (shape.vellen * 0.1f))); modified the air Glide
	...
	if (processSticking)
	{
		...
		this.server_Die(); //Attempt to make the arrows die when hit map
	}
...
void onDie(CBlob@ this)
{
	if (getNet().isClient())
	{
		...
		if (pos.x >= 1 && pos.y >= 1)
		{
			...
			makeGibParticle("GenericGibs2.png", pos, vel,
...
void onHitBlob(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitBlob, u8 customData)
{
	...
	if (this !is hitBlob && customData == Hitters::arrow)
	{
		...
		if (speed > ArcherParams::shoot_max_vel * 0.5f)
		{
			...
			// I don't think we need this stun at the mod, it cancels the bow/rifle charge when hit and feels unbalanced
