...
void makeLargeExplosionParticle(Vec2f pos, string path)
{
	ParticleAnimated(path,
	                 pos, Vec2f(0, 0.5f), 0.0f, 1.0f,
	                 3 + XORRandom(3),
	                 -0.1f, true);
}
...
void Explode(CBlob@ this, f32 radius, f32 damage)
{
	...
	string path = "Entities/Effects/Sprites/Explosion.png";

	if (this.exists("custom_explosion_sprite"))
	{
		path = this.get_string("custom_explosion_sprite");
	}
	...
	if (hitter == Hitters::emp)
	{
		int tilesr = (r / map.tilesize) * 0.5f;
		EMP(this, tilesr, tilesr, 0.0f);
		return;
	}

	//

	makeLargeExplosionParticle(pos, path);
					...
					for (int i = 0; i < 4; i++)
					{
						...
						if (dist < map_damage_radius)
						{
							...
							if (canHit)
							{
								...
								if (canExplosionDamage(map, tpos, tile, hitter))
								{
									if (!map.isTileBedrock(tile))
									{
										if (dist >= rad_thresh ||
										        !canExplosionDestroy(map, tpos, tile, hitter))
										{
...
{
	if (customData == Hitters::plasma || customData == Hitters::bomb || customData == Hitters::emp)
	{
	...
	for (int step = 0; step <= steps; ++step)
	{
		...
		for (int width_step = 0; width_step < width_steps + 2; width_step++)
		{
			...
			if (isserver)
			{
				...
				{
					if (canExplosionDamage(map, tpos, t, hitter))
					{
						if (!justhurt)
							damaged = true;

						justhurt = justhurt || !canExplosionDestroy(map, tpos, t, hitter);
...
bool canExplosionDamage(CMap@ map, Vec2f tpos, TileType t, const u8 hitter)
{
	bool can_damage = t != CMap::tile_ground_d0 && t != CMap::tile_stone_d0; //don't _destroy_ ground, hit until its almost dead tho

	if (hitter == Hitters::plasma)
	{
		can_damage = map.isTileWood(t);
	}

	return map.getSectorAtPosition(tpos, "no build") is null && can_damage; 
}

bool canExplosionDestroy(CMap@ map, Vec2f tpos, TileType t, const u8 hitter)
{
	if (hitter == Hitters::plasma)
	{
		return map.isTileWood(t);
	}
