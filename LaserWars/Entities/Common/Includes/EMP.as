...
void EMP( CBlob@ this, const uint splash_halfwidth, const uint splash_halfheight, const f32 splash_offset )
{
	//extinguish fire
	...
	if (map !is null)
	{
		...
		if (is_server)
		{
			...
			for (uint i = 0; i < blobs.length; i++)
			{
				...
				if (ownerBlob is blob || (this.isOverlapping(blob) && hitHard))
				{
					this.server_Hit(blob, pos, bombforce, 0.0f, Hitters::emp_force, true);
				}

				if (hitHard)
				{
					u8  shield = getStat(blob, Stats::SHIELD);
					if (shield > 0)
					{
						server_SetStat(blob, Stats::SHIELD, 0, 90);
					}

					this.server_Hit(blob, pos, bombforce, 0.0f, Hitters::emp, true);
				}
			}
		}
	}
