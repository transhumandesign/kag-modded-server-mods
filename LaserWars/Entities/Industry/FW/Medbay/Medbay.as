...
const f32 beer_amount = 25.0f;
const f32 heal_amount = 5.0f;
const u8 heal_rate = 30;

void onInit(CSprite@ this)
{
	CSpriteLayer@ bed = this.addSpriteLayer("bed", "Medbay.png", 32, 16);
	if (bed !is null)
	{
		{
	...
	CSpriteLayer@ zzz = this.addSpriteLayer("zzz", "Medbay.png", 8, 8);
	if (zzz !is null)
	{
		{
	...
	CSpriteLayer@ backpack = this.addSpriteLayer("backpack", "Medbay.png", 16, 16);
	if (backpack !is null)
	{
		{
...
void onInit(CBlob@ this)
{
	...
	// ICONS
	AddIconToken("$quarters_beer$", "Medbay.png", Vec2f(24, 24), 7);
	AddIconToken("$quarters_meal$", "Medbay.png", Vec2f(48, 24), 2);
	AddIconToken("$quarters_egg$", "Medbay.png", Vec2f(24, 24), 8);
	AddIconToken("$quarters_burger$", "Medbay.png", Vec2f(24, 24), 9);
	...
	{
		ShopItem@ s = addShopItem(this, "Beer - 25 Health", "$quarters_beer$", "beer", "A refreshing mug of beer.", false);
		s.spawnNothing = true;
	...
	{
		ShopItem@ s = addShopItem(this, "Burger - 50 Health", "$quarters_burger$", "food", "A burger to go.", true);
		...
		if (patient !is null)
		{
			...
			else if (getGameTime() % heal_rate == 0)
			{
				if (requiresTreatment(patient))
				{
					...
					if (isServer)
					{
					    server_SetStat(patient, Stats::HEALTH, getStat(patient, Stats::HEALTH) + heal_amount, 0, false);
					}
				}
				else
				{
	...
	if (cmd == this.getCommandID("shop made item"))
	{
		...
		string name = params.read_string();
		{
			...
			if (name == "beer")
			{
				...
				if (isServer)
				{
					server_SetStat(callerBlob, Stats::HEALTH, getStat(callerBlob, Stats::HEALTH) + beer_amount, 0, false);
				}
			}
			else if (name == "meal")
			{
				...
				if (isServer)
				{
					server_SetStat(callerBlob, Stats::HEALTH, getBaseStat(callerBlob, Stats::HEALTH), 0, false);
				}
			}
		}
	}
...
{
	return getStat(caller, Stats::HEALTH) < getBaseStat(caller, Stats::HEALTH);
}
