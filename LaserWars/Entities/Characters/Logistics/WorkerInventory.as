...
void onInit(CInventory@ this)
{
	...
	AddIconToken("$BUILDER_CLEAR$", "WorkerIcons.png", Vec2f(32, 32), 2);

	for(u8 i = 0; i < Worker::PAGE_COUNT; i++)
	{
		AddIconToken("$"+PAGE_NAME[i]+"$", "WorkerPageIcons.png", Vec2f(48, 24), i);
	}
...
void MakeBlocksMenu(CInventory@ this, const Vec2f &in INVENTORY_CE)
{
	...
	if(menu !is null)
	{
		...
		if(tool !is null)
		{
			...
			CGridButton@ clear = tool.AddButton("$BUILDER_CLEAR$", "", Worker::TOOL_CLEAR, Vec2f(1, 1), params);
			if(clear !is null)
			{
		...
		const Vec2f INDEX_POS = Vec2f(menu.getLowerRightPosition().x + GRID_PADDING + GRID_SIZE, menu.getUpperLeftPosition().y + GRID_SIZE * Worker::PAGE_COUNT / 2);

		CGridMenu@ index = CreateGridMenu(INDEX_POS, blob, Vec2f(2, Worker::PAGE_COUNT), "Type");
		if(index !is null)
		{
			...
			for(u8 i = 0; i < Worker::PAGE_COUNT; i++)
			{
				CGridButton@ button = index.AddButton("$"+PAGE_NAME[i]+"$", PAGE_NAME[i], Worker::PAGE_SELECT + i, Vec2f(2, 1), params);
				if(button is null) continue;
...
void onCreateInventoryMenu(CInventory@ this, CBlob@ forBlob, CGridMenu@ menu)
{
	...
	string weapon = getActiveItem(blob);
...
{
	string dbg = "WorkerInventory.as: Unknown command ";

	CBlob@ blob = this.getBlob();
	if(blob is null) return;

	if(cmd >= Worker::make_block && cmd < Worker::make_reserved)
	{
	...
	else if(cmd >= Worker::PAGE_SELECT && cmd < Worker::PAGE_SELECT + Worker::PAGE_COUNT)
	{
		u16 id;
		if(!params.saferead_u16(id)) return;
		...
		target.set_u8("build page", cmd - Worker::PAGE_SELECT);
