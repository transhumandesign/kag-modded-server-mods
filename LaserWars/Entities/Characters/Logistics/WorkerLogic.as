...
void onInit( CBlob@ this )
{
	...
	this.set_f32("tool_distance", 15.0f);
...
void SendHitCommand( CBlob@ this, CBlob@ blob, const Vec2f tilepos, const Vec2f attackVel, const f32 attack_power )
{
	...
	this.SendCommand(this.getCommandID("tool"), params);
}
...
bool RecdHitCommand( CBlob@ this, CBitStream@ params )
{
	...
	else
	{
		...
		if (blob !is null)
		{
			...
			this.server_Hit(blob, tilepos, attackVel, blob.hasTag("flesh") ? attack_power * 2 : attack_power, Hitters::builder, teamHurt);
		}
	}
...
{
	if(cmd == this.getCommandID("tool"))
	{
		if(!RecdHitCommand(this, params))
			warn("error when recieving tool command");
	}
}
...
void Drill( CBlob@ this )
{
	HitData@ hitdata;
	CSprite@ sprite = this.getSprite();

	CSpriteLayer@ tool = sprite.getSpriteLayer("tool_layer");
	
	if (getActiveItem(this) != "omni_tool" || tool is null || !tool.isAnimation("spinning"))
	{
		this.get("hitdata", @hitdata);
		hitdata.blobID = 0;
	...
	bool justCheck = !tool.isFrameIndex(hit_frame);
	bool adjusttime = tool.getFrameIndex() < hit_frame - 1;
	...
	f32 attack_distance = this.getRadius() + this.get_f32("tool_distance");
	...
	const f32 tile_attack_distance = attack_distance;
