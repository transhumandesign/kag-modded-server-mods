...
void onTick(CMovement@ this)
{
	...
	// jetpack - overrides other movement completely
	if (blob.hasTag("jetpack") && !isknocked)
	{
		Vec2f ladderforce;

		if(!up && !down)
		{
		    ladderforce.y -= 0.6f; // 0.75f
		}

		if (up)
		{
			ladderforce.y -= 1.4f; // 1.2f
		}
		...
		blob.AddForce(ladderforce * moveVars.overallScale * 50.0f); // * 50.0f
		//damp vel
		Vec2f vel = blob.getVelocity();
		vel *= 0.65f; // *= 0.65f
