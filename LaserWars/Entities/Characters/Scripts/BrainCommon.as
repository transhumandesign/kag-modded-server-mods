...
CBlob@ getClosestTarget( CBrain@ this, CBlob@ blob )
{
	...
	{
		CBlob@ candidate = players[i];

		f32 distance = (candidate.getPosition() - pos).getLength();
		if (distance < best_dist)
		{
			if (candidate !is blob && blob.getTeamNum() != candidate.getTeamNum() && !candidate.hasTag("dead") && !candidate.hasTag("cloaked"))
			{
				blob.set_Vec2f("last pathing pos", candidate.getPosition());
...
void DefaultChaseBlob(CBlob@ blob, CBlob@ target)
{
	...
	if (XORRandom(30) == 0 && (blob.get_Vec2f("last pathing pos") - targetPos).getLength() > 50.0f)
	{
		Repath(brain);
	...
	{
		if (state == CBrain::has_path && targetDistance < 140.0f && !isVisible(blob, target))
		{
...
void ClosestTarget( CBrain@ this )
{
	CBlob@ blob = this.getBlob();
	CBlob@ target = this.getTarget();

	if (target is null || (getGameTime() + blob.getNetworkID()) % 30 == 0)
	{
		CBlob@ oldTarget = target;
		@target = getClosestTarget(this, blob);
