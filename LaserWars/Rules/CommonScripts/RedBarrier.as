...
const f32 BARRIER_PERCENT = 0.125f;
	...
	if (shouldBarrier(this))
	{
		...
		if (getMap().getBlobsInBox(Vec2f(x1, y1), Vec2f(x2, y2), @blobsInBox))
		{
			for (uint i = 0; i < blobsInBox.length; i++)
			{
				...
				if (b.hasTag("projectile"))
				{
					b.server_Die();
				}
				else if (b.getTeamNum() < 100 || b.hasTag("no barrier pass"))
				{
...
void onRender(CRules@ this)
{
	if (shouldBarrier(this))
	{
		f32 x1, x2, y1, y2;
		getBarrierPositions(x1, x2, y1, y2);
		GUI::DrawRectangle(getDriver().getScreenPosFromWorldPos(Vec2f(x1, y1)), getDriver().getScreenPosFromWorldPos(Vec2f(x2, y2)), SColor(75, 235, 0, 0));
	}
}
