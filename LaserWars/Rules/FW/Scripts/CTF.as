...
{
	string configstr = "../Mods/" + MOD_FOLDER + "/Rules/FW/fw_vars.cfg";
	...
	s32 warmUpTimeSeconds = cfg.read_s32("warmup_time", 240);
...
shared class CTFCore : RulesCore
{
	...
	{
		string blob = player.isBot() ? "soldier" : "logistics";
		...
		if (player !is null && !player.isBot())
		{
			...
			if (profile !is null)
			{
				blob = profile.loadout_exosuit == "logistics" ? "logistics" : "soldier";
			}
		}
		...
		CTFPlayerInfo p(player.getUsername(), team, blob);
		players.push_back(p);
