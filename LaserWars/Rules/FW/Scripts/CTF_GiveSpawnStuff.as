...
bool GiveSpawnResources(CRules@ this, CBlob@ blob, CPlayer@ player, CTFPlayerInfo@ info)
{
	...
	else if (blob.getName() == "soldier")
	{
	    string weapon = getItem(blob, ItemType::PRIMARY);

	    if (weapon == "plasma_launcher" || weapon == "plasma_cannon")
	    {
	    	ret = SetMaterials(blob, "mat_cells", 6) || ret;
	    }
	    else if (weapon == "swarm_launcher")
	    {
	    	ret = SetMaterials(blob, "mat_swarms", 4) || ret;
	    }
...
void onTick(CRules@ this)
{
	...
	if (core !is null)
	{
		...
		getBlobsByName("munitions", @spots);
...
void onRender(CRules@ this)
{
	...
	if (b !is null && this.exists(propname))
	{
		...
		{
			f32 offset = 140.0f;
			string verb = (b.getName() == "logistics" ? "Build" : "Fight");
			if (this.isWarmup())
			{
