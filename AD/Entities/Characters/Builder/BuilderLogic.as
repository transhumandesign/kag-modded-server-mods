...
f32 adjustHitDamage(CBlob@ this, CBlob@ blob, f32 dam)
{
    string name = blob.getName();
    if(name.findFirst("shop") != -1 || name == "building" || name == "ladder")
    {
        return 0.25f;

    }
    else if(name == "saw" || name == "trampoline")
    {
    ...
    else if(name.findFirst("door") != -1)
    {
        return 0.5f;

    }
    else if(name == "ballista" || name == "catapult" || name == "drill")
    {
        return 0.5f;
	...
	if (!adjusttime)
	{
		if (!justCheck)
		{
			...
			else
			{
				...
				if (b !is null)
				{
                    f32 dam = adjustHitDamage(this, b, hit_damage);
					SendHitCommand(this, b, (b.getPosition() + this.getPosition()) * 0.5f, attackVel, dam);
				}
			}
		}
		return;
