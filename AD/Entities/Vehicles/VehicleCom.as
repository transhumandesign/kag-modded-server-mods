...
{						
	int ammoStocked = v.loaded_ammo - v.fire_amount;
...
void Vehicle_LoadAmmoIfEmpty( CBlob@ this, VehicleInfo@ v )
{
    ...
    {												   
		...
		if (toLoad !is null)
		{
			...
			{
				server_LoadAmmo(this, toLoad, v.fire_amount, v);
			}
		...
		{
			server_LoadAmmo(this, null, v.fire_amount, v);
		}
    }
}
...
{
    // normal fire
    if ( canFire(this, v) && caller !is null ) // can fire
    {
        ...
        if (shot) {
            this.getSprite().PlaySound( CFileMatcher(v.fire_sound).getRandom() );
        }
        else
        {   // empty shot
            this.getSprite().PlaySound( CFileMatcher(v.empty_sound).getRandom() );
    ...
    if (this.getAttachmentPoints( @aps ))
    {
        for (uint i = 0; i < aps.length; i++)
        {
            ...
            if (blob !is null && ap.socket)
            {
				...
				if ( (ap.name == "ROWER" && this.isInWater()) || (ap.name == "SAIL" && !this.hasTag("no sail") && this.isInWater()))
				{
					const f32 moveForce = ((v.move_speed+10) * 2);
