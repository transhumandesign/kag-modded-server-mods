...
void onInit(CBlob@ this)
{
	...
	if (this.exists("frame"))
	{
		...
		{
            //this.getShape().RemoveShape(0);
            if(this.getShape() !is null)
            {
                Vec2f[] bigshape = { Vec2f(-6, 0), Vec2f(22, 0), Vec2f(22, 20), Vec2f(-6, 20) };
                this.getShape().AddShape(bigshape); 

            }

            CSpriteLayer@ layer = this.getSprite().addSpriteLayer("big", "Crate.png", 32, 32);
            Animation@ a = layer.addAnimation("biganim", 0, false);
            a.AddFrame(6);
            layer.SetOffset(Vec2f(0, -2));

            this.getAttachments().getAttachmentPointByName("PICKUP").offset.y = 10;
			...
			if (icon !is null)
			{
				...
				icon.SetOffset(Vec2f(-2, 4));
				icon.SetRelativeZ(1);
			}

			this.getSprite().SetAnimation("poof");
...
void ShowParachute(CBlob@ this)
{
	...
	if (packed == "catapult" || packed == "bomber" || packed == "ballista" || packed == "mounted_bow" || packed == "longboat" || packed == "warboat")	 // HACK:
	{
	    ...
	    if (parachute !is null)
	    {
		    ...
		    anim.AddFrame(4);
		    parachute.SetOffset(Vec2f(-14.0f, - 17.0f));
	    }

	    CSpriteLayer@ parachute2 = sprite.addSpriteLayer("parachute2",   32, 32);

	    if (parachute2 !is null)
	    {
		    Animation@ anim = parachute2.addAnimation("default", 0, true);
		    anim.AddFrame(4);
		    parachute2.SetOffset(Vec2f(14.0f, - 17.0f));
	    }
...
void HideParachute(CBlob@ this)
{
	...
	if (packed == "catapult" || packed == "bomber" || packed == "ballista" || packed == "mounted_bow" || packed == "longboat" || packed == "warboat")	 // HACK:
	{
	    ...
	    CSpriteLayer@ parachute2 = sprite.getSpriteLayer("parachute2");

	    if (parachute2 !is null && parachute2.isVisible())
	    {
		    parachute2.SetVisible(false);
		    ParticlesFromSprite(parachute2);
	    }
...
{
    //fuck off mate
    if(attached.getTeamNum() != this.getTeamNum())
    {
	    this.server_Hit(attached, attached.getPosition(), Vec2f(1, 1), 0.5, Hitters::fire);
