...
void onCollision(CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point1)
{
	if (blob !is null && doesCollideWithBlob(this, blob) && !this.hasTag("collided"))
	{
		if (!solid && !blob.hasTag("flesh") && !specialArrowHit(blob) &&
		        (blob.getName() != "mounted_bow" || this.getTeamNum() != blob.getTeamNum()) && blob.getName() != "princess")
		{
			return;
		}
		...
		else
		{
            ...
            if(this.hasTag("frombolter") && blob.getName() != "ballista" && blob.getName() != "catapult")
            {
                dmg = 0.5f;

            }

            if(blob.getName() == "platform" && arrowType != ArrowType::fire)
                dmg = 0.0f;
...
{
    if(blob.getName() == "ballista_bolt" || blob.getName() == "arrow")
        return false;
